...
Page({
  ...
  formSubmit: function (e){
    ...
    if ( e.detail.value.school.length == 0 || e.detail.value.grade.length == 0 ||  e.detail.value.tel.length == 0 || e.detail.value.qq.length == 0 || e.detail.value.sushe.length == 0 ) {
      wx.showToast({
        title: '不能留空！',
        icon:'none',
        duration: 2000
      ...
      wx.getStorage({
        ...
        success: function(res) {
          wx.request({
            url: 'https://1758541396.cn/userupdate.php?id='+res.data,
            data: {
              school: data.school,
              grade: data.grade,
              tel: data.tel,
